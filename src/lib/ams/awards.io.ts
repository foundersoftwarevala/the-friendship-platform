...
export function toJSON(rows: Award[]): string {
  return JSON.stringify(
    rows.map((a) => ({
      ...
      rewards: a.rewards,
      media: a.media,
      supportedModules: a.supportedModules,
...
export function fromCSV(text: string, defaultType: AwardType): AwardDraft[] {
  ...
  const idx = (key: string) =>
    header.findIndex((h) => h.trim().toLowerCase() === key.toLowerCase());
  const i = Object.fromEntries(CSV_COLUMNS.map((c) => [c, idx(c)])) as Record<
    ...
    .map((r): AwardDraft | null => {
      ...
      const draft: AwardDraft = {
        ...
        rewards: {
          ...
          rankImpact: 0,
          levelImpact: 0,
          monetaryValue: num(r[i.monetaryValue]),
        },
        media: { themeColor: (r[i.themeColor] ?? "").trim() || undefined },
        supportedModules: (r[i.supportedModules] ?? "")
          .split("|")
          .map((s) => s.trim())
          .filter(Boolean),
        supportedRoles: (r[i.supportedRoles] ?? "")
          .split("|")
          .map((s) => s.trim())
          .filter(Boolean),
      };
...
export function fromJSON(text: string, defaultType: AwardType): AwardDraft[] {
  ...
  return arr
    .filter(
      (x: unknown): x is { name: string } & Partial<Award> =>
        !!x &&
        typeof x === "object" &&
    ...
    .map((x): AwardDraft => ({
      ...
      rewards: {
        xp: x.rewards?.xp ?? 0,
        coins: x.rewards?.coins ?? 0,
        rankImpact: x.rewards?.rankImpact ?? 0,
        levelImpact: x.rewards?.levelImpact ?? 0,
...
export const BULK_STATUSES: AwardStatus[] = [
  "published",
  "unpublished",
  "archived",
  "approved",
