...
export const getCommandCenter = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [
    ...
    supabase
      ...
      .select("id, achievement_id, unlocked_at, achievements(name, rarity, icon, color)")
      .eq("user_id", userId)
      .order("unlocked_at", { ascending: false })
      .limit(50),
    supabase
      .from("user_badges")
      .select("id, badge_id, earned_at, badges(name, rarity, icon, color)")
      .eq("user_id", userId)
      .order("earned_at", { ascending: false })
      .limit(50),
    supabase
      .from("user_trophies")
      .select("id, trophy_id, earned_at, trophies(name, tier, icon, color)")
      .eq("user_id", userId)
      .order("earned_at", { ascending: false })
      .limit(50),
    ...
    supabase
      .from("missions")
      .select("id, name, description, xp_reward, ends_at")
      .eq("cadence", "daily")
      .eq("status", "active")
      .limit(6),
    supabase
    ...
    supabase
      .from("claims")
      .select("id, status, created_at, cost_coins, cost_tokens, rewards(name, icon)")
      .eq("user_id", userId)
      .order("created_at", { ascending: false })
      .limit(10),
    supabase
      .from("user_achievements")
      .select("unlocked_at, achievements(name, rarity, icon)")
      .eq("user_id", userId)
    ...
    supabase
      .from("user_badges")
      .select("earned_at, badges(name, rarity, icon)")
      .eq("user_id", userId)
      .order("earned_at", { ascending: false })
      .limit(8),
    supabase
      .from("user_trophies")
      .select("earned_at, trophies(name, tier, icon)")
      .eq("user_id", userId)
      .order("earned_at", { ascending: false })
      .limit(8),
    supabase
      .from("claims")
      .select("created_at, status, rewards(name, icon)")
      .eq("user_id", userId)
    ...
    supabase
      ...
      .select("id, name, slug, scope, metric")
      .eq("status", "active"),
    supabase
      .from("notifications")
      .select("id", { count: "exact", head: true })
  ...
  const currentLevel =
    [...levels].reverse().find((l) => totalXp >= l.xp_required) ?? levels[0] ?? null;
  ...
  const entrySelect =
    "rank, score, user_id, profiles(display_name, avatar_url, country, role_title)";
      ...
      levelProgressPct:
        currentLevel && nextLevel && nextLevel.xp_required > currentLevel.xp_required
          ? Math.min(
              ...
              Math.max(
                0,
                ((totalXp - currentLevel.xp_required) /
      ...
      rankProgressPct:
        currentRank && nextRank && nextRank.min_xp > currentRank.min_xp
          ? Math.min(
              ...
              Math.max(
                0,
                ((totalXp - currentRank.min_xp) / (nextRank.min_xp - currentRank.min_xp)) * 100,
              ),
            )
          : nextRank
            ? 0
