      ...
      .on(
        ...
        async (payload) => {
          ...
          if (session?.user_id !== user.id && session?.support_agent_id !== user.id) {
            ...
            if (
              !["safe_assist", "assist_manager", "super_admin", "master"].includes(userRole || "")
            ) {
              return;
            }
          }
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (["safe_assist", "assist_manager"].includes(userRole || "")) {
              await addNotification(
                "info",
                `New Safe Assist session requested`,
                "safe_assist_new_session",
                {
            ...
            if (session.status === "connected" && session.user_id === user.id) {
              await addNotification(
                "success",
                "Support agent has connected to your session",
                "safe_assist_connected",
                {
                  isBuzzer: false,
                },
              );
              ...
              await addNotification(
                "danger",
                "Session was terminated due to security concerns",
                "safe_assist_terminated",
                {
                  isBuzzer: true,
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (alert.risk_level === "critical") {
            await addNotification(
              "danger",
              `Critical AI alert: ${alert.event_type}`,
              "safe_assist_critical_alert",
              {
                isBuzzer: true,
            ...
            await addNotification(
              "warning",
              `High risk detected: ${alert.event_type}`,
              "safe_assist_high_alert",
              {
                actionLabel: "Review",
                roleTarget: ["safe_assist", "assist_manager"],
              },
