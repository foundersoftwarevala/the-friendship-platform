...
export function useGlobalActions() {
  ...
  const logToAudit = useCallback(
    async (action: string, module: string, meta: Record<string, any>) => {
      try {
  ...
  const executeAction = useCallback(
    async (payload: ActionPayload): Promise<ActionResult> => {
  ...
  const quickActions = {
    ...
    create: (entityType: EntityType, data: Record<string, any>) =>
      executeAction({
        actionId: `create_${entityType}_${Date.now()}`,
        actionType: "create",
        entityType,
        metadata: data,
        successMessage: `${entityType} created`,
      }),

    update: (entityType: EntityType, entityId: string, data: Record<string, any>) =>
      executeAction({
        actionId: `update_${entityId}`,
        actionType: "update",
        entityType,
        entityId,
        metadata: data,
    ...
    delete: (entityType: EntityType, entityId: string, soft: boolean = true) =>
      executeAction({
        actionId: `delete_${entityId}`,
        actionType: "delete",
        entityType,
        entityId,
        metadata: { soft },
        requiresConfirmation: true,
        successMessage: soft ? "Archived" : "Deleted",
      }),
    ...
    navigate: (target: string, meta?: Record<string, any>) =>
      executeAction({
        ...
        actionType: "navigate",
        entityType: "module" as EntityType,
    ...
    export: (
      ...
      executeAction({
        actionId: `export_${entityType}_${format}`,
        actionType: "export",
        entityType,
        metadata: { format, filters },
        successMessage: `${entityType} exported as ${format.toUpperCase()}`,
      }),

    refresh: (entityType: EntityType) =>
      executeAction({
        actionId: `refresh_${entityType}`,
        actionType: "refresh",
        entityType,
    ...
    toggle: (entityType: EntityType, entityId: string, field: string, currentValue: boolean) =>
      executeAction({
        actionId: `toggle_${field}_${entityId}`,
        actionType: "toggle",
        entityType,
        entityId,
        metadata: { field, oldValue: currentValue, newValue: !currentValue },
        successMessage: `${field} ${!currentValue ? "enabled" : "disabled"}`,
      }),
  };
