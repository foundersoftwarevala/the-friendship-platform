...
export function useDemoManagerAccess(): UseDemoManagerAccessReturn {
  ...
  const logUnauthorizedAttempt = useCallback(
    async (action: string, demoId?: string) => {
      if (!user) return;
  ...
  const createReportCard = useCallback(
    async (params: CreateReportCardParams): Promise<string | null> => {
      if (!user) {
  ...
  const updateWorkflowStatus = useCallback(
    async (reportCardId: string, status: string): Promise<boolean> => {
      if (!isDemoManager) {
