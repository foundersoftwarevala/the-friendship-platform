...
import { Trophy, Award as AwardIcon, Crown, Sparkles, Save, Send, X } from "lucide-react";
...
import { createAward, updateAward, publishAward } from "@/lib/ams/awards.api";
import { toast } from "sonner";

const TYPES: AwardType[] = ["trophy", "badge", "achievement", "rank", "milestone", "streak"];
const VISIBILITIES: AwardVisibility[] = [
  "public",
  "private",
  "role-restricted",
  "module-restricted",
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6">
      <div className="space-y-6">
        <Tabs defaultValue="basics">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. Top Affiliate of the Quarter"
                />
              </div>
              <div className="md:col-span-2 space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.category}
                  onValueChange={(v) => patch({ category: v as AwardCategory })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    patch({ department: v === "_none" ? undefined : (v as Department) })
                  }
                >
                  <SelectTrigger>
          ...
          <TabsContent value="rules" className="surface-card p-5 space-y-4 mt-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-3">
                All conditions must be true for the award to fire.
              </p>
              <RuleBuilder
                value={form.unlockConditions}
                onChange={(c) => patch({ unlockConditions: c })}
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-border">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    patch({
                      supportedModules: e.target.value
                        .split(",")
                        .map((s) => s.trim())
                        .filter(Boolean),
                    })
          ...
          <TabsContent
            value="rewards"
            className="surface-card p-5 grid grid-cols-2 md:grid-cols-3 gap-4 mt-4"
          >
            {(
              [
                ["xp", "XP", Sparkles],
              ...
              <div key={key} className="space-y-1.5">
                <Label className="flex items-center gap-1.5">
                  <Icon className="h-3.5 w-3.5 text-trophy" /> {label}
                </Label>
                <Input
                  type="number"
                  value={form.rewards[key]}
                  onChange={(e) =>
                    patch({ rewards: { ...form.rewards, [key]: Number(e.target.value) } })
                  }
                />
              </div>
            ))}
            <div className="space-y-1.5">
              ...
              <Input
                value={form.rewards.currency ?? ""}
                onChange={(e) =>
                  patch({ rewards: { ...form.rewards, currency: e.target.value || undefined } })
                }
                placeholder="USD"
              />
            </div>
          ...
          <TabsContent value="visibility" className="surface-card p-5 space-y-4 mt-4">
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => patch({ visibility: v as AwardVisibility })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <p className="text-xs text-muted-foreground">
                Public awards appear in catalogs. Role/module-restricted awards only render for
                matching audiences.
              </p>
            </div>
          </TabsContent>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-border">
          <Button
            variant="outline"
            onClick={() => navigate({ to: "/ams/awards" })}
            className="gap-1.5"
          >
