...
export function RuleBuilder({
  ...
  const update = (next: UnlockCondition[]) => {
    setRows(next);
    onChange(next);
  };
  return (
    <div className="space-y-2">
      ...
      {rows.map((row, i) => (
        <div key={row.id} className="grid grid-cols-[1fr_90px_1fr_auto] gap-2">
          <Input
            ...
            onChange={(e) =>
              update(rows.map((r) => (r.id === row.id ? { ...r, metric: e.target.value } : r)))
            }
          />
          <Select
            value={row.operator}
            onValueChange={(v) =>
              update(
                rows.map((r) =>
                  r.id === row.id ? { ...r, operator: v as UnlockCondition["operator"] } : r,
                ),
              )
            }
          >
          ...
          {i < rows.length - 1 && (
            <div className="col-span-4 text-center text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
              AND
            </div>
          )}
        </div>
      ...
      <Button
        variant="outline"
        size="sm"
        onClick={() => update([...rows, { id: uid(), metric: "", operator: ">=", value: 0 }])}
        className="gap-1.5"
      >
