    ...
    <div className="surface-card p-4 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-medium">
          <span className="text-trophy">{icon}</span>
          {label}
        </div>
        {local && (
          <Button
...
export function MediaUploader({
  ...
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-3", className)}>
      <Field
        label="3D Model / Image"
        icon={<ImageIcon className="h-4 w-4" />}
        hint=".glb, .png, .jpg — preview on award detail"
        accept=".glb,.gltf,image/*"
        value={value.model3dUrl}
        onChange={(v) => patch({ model3dUrl: v })}
      />
      <Field
        label="Animated Icon"
        icon={<Sparkles className="h-4 w-4" />}
        hint="GIF or animated SVG"
        accept="image/gif,image/svg+xml"
        value={value.animatedIconUrl}
        onChange={(v) => patch({ animatedIconUrl: v })}
      />
      <Field
        label="Lottie Animation"
        icon={<Sparkles className="h-4 w-4" />}
        hint=".json (Bodymovin export)"
        accept="application/json"
        value={value.lottieUrl}
        onChange={(v) => patch({ lottieUrl: v })}
      />
      <Field
        label="GIF Animation"
        icon={<Film className="h-4 w-4" />}
        hint=".gif for unlock fallback"
        accept="image/gif"
        value={value.gifUrl}
        onChange={(v) => patch({ gifUrl: v })}
      />
      <Field
        label="Sound Effect"
        ...
        onChange={(v) => patch({ soundUrl: v })}
      />
      <div className="surface-card p-4 space-y-2">
        <Label className="text-sm font-medium flex items-center gap-2">
          <span className="text-trophy">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            onChange={(e) => patch({ themeColor: e.target.value || undefined })}
            placeholder="#c9a86a"
          />
        </div>
      </div>
