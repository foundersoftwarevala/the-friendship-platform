...
import { toCSV, toJSON, fromCSV, fromJSON, downloadBlob } from "@/lib/ams/awards.io";
...
export function LibraryPage({
  ...
  async function handleBulk(
    action: "publish" | "unpublish" | "archive" | "approve" | "disable" | "delete",
  ) {
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuLabel>
                  Export {someSelected ? `${selected.size} selected` : "all rows"}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                {someSelected && (
                  <>
                    <DropdownMenuItem onClick={() => exportRows("csv", "selected")}>
                      Selected as CSV
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => exportRows("json", "selected")}>
                      Selected as JSON
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                  </>
                )}
                <DropdownMenuItem onClick={() => exportRows("csv", "all")}>
                  All as CSV
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportRows("json", "all")}>
                  All as JSON
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
