  ...
  return (
    <div className="surface-card relative overflow-hidden p-8">
      ...
      <div className="relative flex flex-col items-center gap-6">
        <div
          className="stage-3d relative grid h-52 w-52 place-items-center overflow-hidden rounded-full"
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(closest-side, color-mix(in oklab, var(--color-primary-glow) 60%, transparent), transparent 74%)",
            }}
          />
          <div className="stage-3d-object relative">
            {award.media.model3dUrl ? (
              <img
                src={award.media.model3dUrl}
                alt={award.name}
                className="h-44 w-44 drop-shadow-[0_20px_50px_color-mix(in_oklab,black_70%,transparent)]"
              />
            ) : (
            ...
            <div className="pointer-events-none absolute inset-0 overflow-hidden">
              <div
                ...
                style={{
                  background:
                    "linear-gradient(100deg, transparent, color-mix(in oklab, white 40%, transparent), transparent)",
                  mixBlendMode: "screen",
                  filter: "blur(2px)",
                }}
              />
        ...
        <div className="text-center">
          <div className="text-[10px] uppercase tracking-[0.3em]" style={{ color: meta.hue }}>
            {meta.label} · {award.type}
          </div>
          <div className="mt-1 font-display text-2xl font-bold text-gradient-trophy">
            {award.name}
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button
            ...
            className="gap-1.5"
            onClick={playSound}
            disabled={!award.media.soundUrl}
          >
            <Volume2 className="h-3.5 w-3.5" /> Play sound
          </Button>
          {award.media.gifUrl && (
            <Button size="sm" variant="outline" className="gap-1.5" asChild>
              <a href={award.media.gifUrl} target="_blank" rel="noreferrer">
                <Play className="h-3.5 w-3.5" /> Open GIF
              </a>
            </Button>
          )}
        </div>
