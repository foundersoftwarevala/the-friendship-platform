...
export function ChatScreen() {
  ...
  async function send() {
    ...
    setMessagesByConv((prev) => ({
      ...
      [active.id]: (prev[active.id] ?? []).map((m) =>
        m.id === optimistic.id ? (data as Message) : m,
      ),
    }));
  }
  ...
  async function startConversation(title: string, allowedRoles: string[]) {
    ...
    const { data: conv, error: convErr } = await supabase
      ...
      .single();
    if (convErr || !conv) {
      setError(convErr?.message ?? "Failed to create channel");
      return;
    }
    ...
    return (
      <div className="gold-frame grid h-[420px] place-items-center rounded-2xl text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          <Lock className="h-4 w-4 text-[var(--color-primary)]" /> Sign in to access secure chat
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
                ...
                <span className="rounded border border-gold px-1.5 text-[10px] uppercase tracking-[0.18em] text-[var(--color-primary-glow)]">
                  {myRole}
                </span>
                .
              </p>
      ...
      {error && (
        <div
          role="alert"
          aria-live="assertive"
          className="motion-shake flex items-start gap-2 rounded-lg border border-destructive/50 bg-destructive/10 px-4 py-2.5 text-xs text-destructive"
        >
        ...
        >
          <div className="border-b border-gold p-4">
            <div className="flex items-center justify-between">
              <div className="text-[10px] uppercase tracking-[0.25em] text-[var(--color-primary)]">
                Inbox
              </div>
              <button
            ...
            <div className="mt-3 flex items-center gap-2 rounded-lg border border-gold bg-surface px-3 py-2">
              ...
              <label htmlFor="chat-channel-search" className="sr-only">
                Search channels
              </label>
              <input
          ...
          <div
            role="group"
            aria-label="Filter channels by role"
            className="flex gap-1 overflow-x-auto border-b border-gold/60 px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            {ROLE_FILTERS.map((r) => (
              <button
                key={r}
            ...
            <>
              <div className="flex items-center justify-between gap-3 border-b border-gold bg-surface px-4 py-3 sm:px-5">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-display text-sm font-semibold text-[var(--color-primary-glow)]">
                        {active.title}
                      </span>
                    ...
                    <div className="mt-0.5 flex items-center gap-2 text-[10px] text-muted-foreground">
                      <Shield className="h-3 w-3 text-accent-emerald" /> Module {active.module} ·
                      realtime enabled
                    </div>
                  </div>
                </div>
                <div className="hidden shrink-0 items-center gap-1 sm:flex">
                  <IconBtn icon={Phone} label="Start voice call" />
                  <IconBtn icon={Video} label="Start video call" />
                  <IconBtn icon={Languages} label="Translate conversation" />
                  <IconBtn icon={Pin} label="Pin conversation" />
                  <IconBtn icon={Bell} label="Notification settings" />
                  <IconBtn icon={MoreVertical} label="More conversation options" />
                </div>
              </div>
              ...
              >
                <div
                  ...
                  style={{
                    backgroundImage:
                      "radial-gradient(circle at 20% 20%, var(--color-primary-glow) 0%, transparent 40%), radial-gradient(circle at 80% 80%, var(--color-primary) 0%, transparent 40%)",
                  }}
                />
              ...
              <div className="border-t border-gold bg-surface px-4 py-3">
                {!canPostHere ? (
                  <div className="flex items-center gap-2 rounded-xl border border-gold/60 bg-surface px-4 py-3 text-xs text-muted-foreground">
                    ...
                    Your role <span className="text-[var(--color-primary-glow)]">{myRole}</span> is
                    not authorized to post here. Allowed: {active.allowed_roles.join(", ")}.
                  </div>
                ) : (
                  <div className="flex items-end gap-2 rounded-2xl border border-gold bg-surface p-2 shadow-[inset_0_1px_0_oklch(0.58_0.19_255/0.15)] focus-within:shadow-[0_0_0_2px_oklch(0.58_0.19_255/0.35)]">
                    <IconBtn icon={Paperclip} small label="Attach a file" />
                    <IconBtn icon={Smile} small label="Insert emoji" />
                    <label
                      htmlFor="chat-composer"
                      className="sr-only"
                    >{`Message ${active.title}`}</label>
                    <textarea
                      id="chat-composer"
        ...
        <aside className="hidden min-h-0 flex-col gap-4 overflow-y-auto border-l border-gold bg-surface p-4 scrollbar-thin xl:flex">
          ...
          {active ? (
            <div className="space-y-3 rounded-xl border border-gold bg-surface p-4 text-xs">
              <div>
                <div className="text-[10px] uppercase tracking-[0.2em] text-[var(--color-primary)]">
                  Title
                </div>
                <div className="mt-1 font-display text-sm text-[var(--color-primary-glow)]">
                  {active.title}
                </div>
              </div>
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {active.allowed_roles.map((r) => (
                    <span
                      key={r}
                      className="rounded border border-gold/60 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.16em] text-[var(--color-primary-glow)]"
                    >
                      {r}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div
                  className={`mt-1 text-[11px] ${canPostHere ? "text-accent-emerald" : "text-red-300"}`}
                >
      ...
      {showNew && (
        <NewConversationModal onClose={() => setShowNew(false)} onStart={startConversation} />
      )}
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`group flex w-full items-start gap-3 rounded-xl p-3 text-left transition-all ${
        active
          ? "bg-[oklch(0.58_0.19_255/0.14)] shadow-[inset_0_0_0_1px_oklch(0.58_0.19_255/0.45)]"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between">
          <span
            className={`truncate text-xs font-semibold ${active ? "text-[var(--color-primary-glow)]" : "text-foreground"}`}
          >
            {conv.title}
          </span>
        </div>
        <div className="mt-0.5 flex flex-wrap items-center gap-1">
          {conv.allowed_roles.slice(0, 3).map((r) => (
            <span
              key={r}
              className="rounded border border-gold/60 px-1.5 py-[1px] text-[9px] uppercase tracking-[0.16em] text-[var(--color-primary)]"
            >
              {r}
            </span>
          ))}
  ...
  return (
    <div className="flex justify-start" role="status" aria-live="polite" aria-atomic="true">
      <span className="sr-only">
        {labels.join(", ")} {labels.length === 1 ? "is" : "are"} typing
      </span>
...
function Dot({ delay }: { delay: string }) {
  return (
    <span
      className="inline-block h-1.5 w-1.5 animate-bounce rounded-full bg-[var(--color-primary-glow)]"
...
function EmptyInbox({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center px-4 py-12 text-center">
      ...
      <div className="mt-4 font-display text-sm font-semibold text-[var(--color-primary-glow)]">
        No channels yet
      </div>
      <p className="mt-1 max-w-xs text-[11px] text-muted-foreground">
        Create a role-scoped channel — only participants with matching roles can post.
      </p>
      <button
        onClick={onNew}
        className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-gold-gradient px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[oklch(0.13_0.025_250)]"
      >
...
function EmptyFilter({ role, onReset }: { role: string; onReset: () => void }) {
  return (
    <div className="px-4 py-8 text-center text-[11px] text-muted-foreground">
      ...
      <button onClick={onReset} className="ml-2 underline hover:text-[var(--color-primary-glow)]">
        Reset
      </button>
    </div>
  );
...
function EmptyStage({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center text-center">
      ...
      <div className="mt-3 font-display text-sm text-[var(--color-primary-glow)]">
  ...
  return (
    <div className="grid h-full place-items-center text-center text-xs text-muted-foreground">
      <div>
        ...
        <div className="mt-2">
          Secure channel <span className="text-[var(--color-primary-glow)]">{title}</span> is ready.
        </div>
      ...
      >
        <div className="flex items-center justify-between border-b border-gold px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2
              ...
              className="font-display text-sm font-semibold text-gold-gradient"
            >
              Create a role-scoped channel
            </h2>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4 p-5">
          <div>
            <label
              ...
              className="text-[10px] uppercase tracking-[0.22em] text-[var(--color-primary)]"
            >
              Channel Title <span aria-hidden="true">*</span>
          ...
          <div>
            ...
            <div
              role="group"
              aria-labelledby="roles-allowed-label"
            ...
            >
              {ALL_POST_ROLES.map((r) => (
                <button
                  ...
                  title={`Toggle ${r}`}
                  onClick={() => toggle(r)}
                  className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] transition-all ${
                    roles.includes(r)
                      ? "bg-gold-gradient text-[oklch(0.13_0.025_250)]"
            ...
            <p id="new-channel-desc" className="mt-1.5 text-[10px] text-muted-foreground">
              Members outside these roles can read but cannot post. Admins always can.
            </p>
          </div>
        </div>
        <div className="flex items-center justify-end gap-2 border-t border-gold bg-surface px-5 py-3">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground hover:text-foreground"
          >
            Cancel
          </button>
          <button
