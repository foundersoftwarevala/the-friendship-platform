    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <VaultToolbar
          ...
          exportLabel={`Export ${showcase.singular.toLowerCase()} set`}
        />
      </div>
      ...
      {(mode === "showcase" || mode === "fullscreen" || mode === "carousel") && (
        <section className="space-y-3">
          <MuseumStage
            {...stageFor(hero, mode === "fullscreen" ? 620 : 480, "full")}
            eager
      ...
      {mode === "comparison" && (
        <section className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Select up to two roles to inspect side by side.
          </p>
          <div className="flex flex-wrap gap-2">
            {visible.map((r) => (
              <button
                ...
                onClick={() =>
                  setCompare((c) =>
                    c.includes(r.slug) ? c.filter((s) => s !== r.slug) : [...c, r.slug].slice(-2),
                  )
                }
            ...
            {(compare.length ? compare : visible.slice(0, 2).map((r) => r.slug)).map((slug) => {
              ...
              return (
                <div key={slug} className="space-y-2">
                  <MuseumStage
                    {...stageFor(role, 420, "full")}
                    onExpand={() => setFullscreenSlug(slug)}
                  />
        ...
        <ol className="relative space-y-6 border-l border-border/60 pl-6">
          {visible.map((role, i) => (
            <li key={role.slug} className="relative">
              <span
                className="absolute -left-[31px] top-4 h-3 w-3 rounded-full"
            ...
            return (
              ...
              >
                <MuseumStage
                  {...stageFor(
                    role,
                    mode === "collection" ? 460 : 360,
                    mode === "museum" ? "full" : "compact",
                  )}
                  onExpand={() => setFullscreenSlug(role.slug)}
                />
                <div className="p-4">
