...
const PLINTH: Record<MuseumEnvironment["plinth"], string> = {
  ...
  glass:
    "linear-gradient(180deg, rgba(226,240,255,0.55) 0%, rgba(140,180,220,0.14) 50%, rgba(10,20,35,0.6) 100%)",
};
...
export function MuseumStage({
  ...
  const imageFilter = useMemo(() => {
    ...
    if (mode === "wireframe")
      return `${glow} grayscale(1) contrast(2.4) brightness(1.5) invert(0.08)`;
    if (mode === "material")
      return `${glow} saturate(1.5) contrast(1.25) brightness(${0.9 + light.intensity * 0.25})`;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(70% 45% at 50% 0%, ${environment.ambient}${Math.round(
            light.intensity * 40,
          )
            .toString(16)
      ...
      <div
        ...
        style={{
          ...
          filter: "blur(14px)",
          opacity: 0.6 + light.intensity * 0.25,
        }}
      />
      {/* cinematic rim sweep */}
      ...
      >
        {!mounted ? (
          ...
          <div className="relative" style={objectStyle}>
            ...
            {explode && (
              <>
                <div
                  className="pointer-events-none absolute inset-0 -translate-y-[26%] scale-[0.72] opacity-45"
                  style={{
                ...
                />
                <div
                  className="pointer-events-none absolute inset-x-6 bottom-[-14%] h-10 rounded-[50%] border"
                  style={{ borderColor: `${accent}77`, background: `${accent}18` }}
                />
                <div
                  className="pointer-events-none absolute inset-x-12 bottom-[-30%] h-8 rounded-[50%] border"
            ...
            {animate && mode !== "wireframe" && (
              <div
                className="trophy-shine pointer-events-none absolute inset-0"
                style={{
                  background: `linear-gradient(115deg, transparent 40%, ${accent}55 50%, transparent 60%)`,
                  mixBlendMode: "screen",
                }}
              />
            )}
        ...
        >
          <div
            ...
            style={{
              background: `linear-gradient(180deg, transparent, ${environment.floor})`,
              opacity: 0.8,
            }}
          />
          <div
            className="absolute bottom-3 left-1/2 h-3 -translate-x-1/2 rounded-full"
            style={{
              width: height * 0.5,
              background: `radial-gradient(closest-side, ${accent}cc, transparent 70%)`,
              filter: "blur(9px)",
            }}
          />
          <div
            className="absolute bottom-0 left-1/2 -translate-x-1/2 rounded-t-[10px] border-t"
            style={{
              width: height * 0.62,
              height: 18,
        ...
        <div
          ...
          style={{
            ...
            background:
              "linear-gradient(115deg, rgba(255,255,255,0.06) 0%, transparent 28%, transparent 72%, rgba(255,255,255,0.045) 100%)",
      ...
      <div
        className="absolute bottom-2 left-3 z-10 font-mono uppercase"
      ...
      <div className="absolute right-3 top-3 z-10 flex flex-col items-end gap-1.5">
        <div className="flex items-center gap-1.5">
          <StageBtn
            ...
            accent={accent}
            title={mode === "auto" ? "Pause auto rotate" : "Auto rotate"}
          >
            {mode === "auto" ? (
          ...
          <StageBtn
            ...
            active={mode === "pan"}
            title="Pan"
            onClick={() => setMode(mode === "pan" ? "auto" : "pan")}
          >
        ...
        {chrome === "full" && (
          <div className="flex flex-wrap justify-end gap-1.5">
            <StageBtn
              ...
              active={mode === "inspect"}
              title="Inspect mode"
              onClick={() => setMode(mode === "inspect" ? "auto" : "inspect")}
            >
              <Search className="h-3.5 w-3.5" />
            </StageBtn>
            <StageBtn
              accent={accent}
              active={mode === "explosion"}
              title="Explosion view"
              onClick={() => setMode(mode === "explosion" ? "auto" : "explosion")}
            >
            ...
            <StageBtn
              ...
              onClick={() => setMode(mode === "wireframe" ? "auto" : "wireframe")}
            >
            ...
            <StageBtn
              ...
              onClick={() => setMode(mode === "material" ? "auto" : "material")}
            >
      ...
      {(unlockTitle || unlockSubtitle) && (
        ...
        >
          ...
          {unlockError && (
            <span className="max-w-40 text-right text-[10px] text-destructive">{unlockError}</span>
          )}
        </div>
      )}
    </div>
...
function StageBtn({
  ...
  return (
    <button
      ...
      style={{
        borderColor: `${accent}66`,
        background: active ? `${accent}44` : "rgba(0,0,0,0.45)",
      }}
    >
      {children}
    </button>
...
export function MuseumFullscreen({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col bg-background/80 p-4 backdrop-blur-xl motion-rise"
      role="dialog"
      aria-modal="true"
