...
export function Row1Totals({ data }: { data: Data }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      <StatCard
        label="Total XP"
        value={<AnimatedNumber value={t.xp} />}
        sub="lifetime"
      ...
      <StatCard
        label="Current Level"
        value={data.progression.currentLevel?.level_number ?? "—"}
        sub={data.progression.currentLevel?.name ?? "no levels yet"}
        icon={<ArrowUpCircle className="h-5 w-5" />}
        accent="primary"
      />
      <StatCard
        label="Current Rank"
        value={data.progression.currentRank?.name ?? "—"}
        sub={
          data.progression.currentRank
            ? `#${data.progression.currentRank.rank_number}`
            : "no ranks yet"
        }
      ...
      <StatCard
        label="Achievements"
        value={`${t.achievementsEarned}/${t.achievementsCatalog}`}
        sub="unlocked / total"
      ...
      <StatCard
        label="Badges"
        value={`${t.badgesEarned}/${t.badgesCatalog}`}
        sub="earned / total"
      ...
      <StatCard
        label="Trophies"
        value={`${t.trophiesEarned}/${t.trophiesCatalog}`}
        sub="earned / total"
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
      <Card title="XP Progress" icon={<Zap className="h-4 w-4 text-xp" />}>
        <div className="text-3xl font-bold tabular-nums text-gradient-primary">
          <AnimatedNumber value={data.totals.xp} duration={800} />
        </div>
        <div className="text-xs text-muted-foreground mt-1">total xp earned</div>
        <div className="mt-3">
          <ProgressBar pct={p.levelProgressPct} />
        </div>
      </Card>
      <Card title="Next Level" icon={<ArrowUpCircle className="h-4 w-4 text-primary" />}>
        <div className="text-3xl font-bold tabular-nums">
          {p.nextLevel ? `L${p.nextLevel.level_number}` : "MAX"}
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {p.nextLevel
            ? `${fmt.format(p.nextLevel.xp_required - data.totals.xp)} XP to ${p.nextLevel.name}`
            : "Top of progression"}
        </div>
        <div className="mt-3">
          <ProgressBar pct={p.levelProgressPct} />
      ...
      <Card title="Rank Progress" icon={<Crown className="h-4 w-4 text-legendary" />}>
        <div className="text-3xl font-bold tabular-nums">
          {p.nextRank ? p.nextRank.name : "APEX"}
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {p.nextRank
            ? `${fmt.format(p.nextRank.min_xp - data.totals.xp)} XP to next rank`
            : "Highest rank achieved"}
        </div>
        <div className="mt-3">
          <ProgressBar pct={p.rankProgressPct} gradient="var(--gradient-trophy)" />
        </div>
      </Card>
      <Card title="Current Streak" icon={<Flame className="h-4 w-4 text-legendary" />}>
        <div className="text-3xl font-bold tabular-nums rarity-legendary">
          {s.current_streak ?? 0}
      ...
      <Card title="Longest Streak" icon={<History className="h-4 w-4 text-trophy" />}>
        <div className="text-3xl font-bold tabular-nums text-gradient-trophy">
          {s.longest_streak ?? 0}
          <span className="text-base text-muted-foreground ml-1">days</span>
        </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      <Card title="Achievement Progress Map" icon={<Trophy className="h-4 w-4 text-trophy" />} fill>
        {data.totals.achievementsCatalog === 0 ? (
          <EmptyHint>
            No achievements configured yet. Create some in the Achievements page.
          </EmptyHint>
        ) : (
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    unlocked
                      ? "bg-trophy/30 border-trophy/60 glow-trophy"
      ...
      <Card title="Badge Collection" icon={<Shield className="h-4 w-4 text-secondary" />} fill>
        {data.collections.badges.length === 0 ? (
          <EmptyHint>No badges earned yet.</EmptyHint>
        ) : (
      ...
      <Card title="Trophy Collection" icon={<Award className="h-4 w-4 text-trophy" />} fill>
        {data.collections.trophies.length === 0 ? (
          <EmptyHint>No trophies earned yet.</EmptyHint>
        ) : (
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      <LeaderboardCard
        ...
        entries={p.global}
        hint="No global leaderboard configured yet."
      />
      <LeaderboardCard
        title="Role Leaderboard"
        icon={<Medal className="h-4 w-4 text-secondary" />}
        entries={p.role}
        hint="Define a role-scoped leaderboard to populate this."
      />
      <LeaderboardCard
        title="Territory Leaderboard"
        icon={<TrendingUp className="h-4 w-4 text-xp" />}
        entries={p.territory}
        hint="Define a territory-scoped leaderboard to populate this."
      />
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <MissionsCard
        ...
        cadenceLabel="Resets daily"
        icon={<Target className="h-4 w-4 text-primary" />}
        items={data.missions.daily}
      />
      <MissionsCard
        title="Weekly Missions"
        cadenceLabel="Resets weekly"
        icon={<Target className="h-4 w-4 text-secondary" />}
        items={data.missions.weekly}
      />
      <MissionsCard
        title="Monthly Missions"
        cadenceLabel="Resets monthly"
        icon={<Target className="h-4 w-4 text-trophy" />}
        items={data.missions.monthly}
      />
      <MissionsCard
        title="Seasonal Missions"
        cadenceLabel="Resets per season"
        icon={<Target className="h-4 w-4 text-legendary" />}
        items={data.missions.seasonal}
      />
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <ListCard
        ...
        icon={<Swords className="h-4 w-4 text-mythic" />}
        items={data.engagement.challenges}
      />
      <ListCard
        title="Active Quests"
        icon={<Compass className="h-4 w-4 text-primary" />}
        items={data.engagement.quests}
      />
      <ListCard
        title="Campaign Progress"
        icon={<Calendar className="h-4 w-4 text-secondary" />}
        items={data.engagement.campaigns}
      />
      <ListCard
        title="Event Progress"
        icon={<CalendarDays className="h-4 w-4 text-trophy" />}
        items={data.engagement.events}
      />
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <StatCard
        ...
        value={fmt.format(pendingClaims)}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      ...
      <Card title="Claim Status" icon={<PackageCheck className="h-4 w-4 text-xp" />}>
        {data.economy.claims.length === 0 ? (
          <EmptyHint>No claims yet.</EmptyHint>
        ) : (
      ...
      <Card title="Reward History" icon={<History className="h-4 w-4 text-muted-foreground" />}>
        {data.timelines.rewards.length === 0 ? (
          <EmptyHint>No reward history yet.</EmptyHint>
        ) : (
          <ul className="text-xs space-y-1.5">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <TimelineCard
        ...
        icon={<Trophy className="h-4 w-4 text-trophy" />}
        items={data.timelines.achievements}
        keyName="achievements"
        dateField="unlocked_at"
      />
      <TimelineCard
        title="Recent Badges"
        icon={<Shield className="h-4 w-4 text-secondary" />}
        items={data.timelines.badges}
        keyName="badges"
        dateField="earned_at"
      />
      <TimelineCard
        title="Recent Trophies"
        icon={<Award className="h-4 w-4 text-trophy" />}
        items={data.timelines.trophies}
        keyName="trophies"
        dateField="earned_at"
      />
      <TimelineCard
...
export function Row10AI({ data }: { data?: Data }) {
  const cards: { title: string; copy: string; result: string }[] = [
    {
      title: "Growth Analysis",
      copy: "XP velocity and progression health from live activity.",
      result: `${data?.xpRecent?.length ?? 0} recent XP events across ${data?.totals?.achievementsEarned ?? 0} achievements.`,
    },
    {
      title: "Next Best Action",
      copy: "The next progression milestone based on current standing.",
      result: data?.progression?.nextLevel
        ? `Reach ${data.progression.nextLevel.name} at ${data.progression.nextLevel.xp_required} XP.`
        : "No higher level is currently configured.",
    },
    {
      title: "Achievement Opportunity",
      copy: "Catalogue gap calculated from earned and available achievements.",
      result: `${Math.max(0, (data?.totals?.achievementsCatalog ?? 0) - (data?.totals?.achievementsEarned ?? 0))} achievements remain available.`,
    },
    {
      title: "Reward Insight",
      copy: "Wallet and claim activity from the persistent reward ledger.",
      result: `${data?.economy?.claims?.length ?? 0} reward claims recorded.`,
    },
  ];
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      {cards.map((c) => (
        <Card key={c.title} title={c.title} icon={<Sparkles className="h-4 w-4 text-secondary" />}>
          ...
          <div className="mt-3 rounded-md border border-border bg-muted/30 p-2 text-xs font-medium text-foreground">
            {c.result}
          </div>
        </Card>
      ))}
...
export function Row11Heatmaps({ data }: { data: Data }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <HeatmapCard
        title="Achievement Heatmap"
        data={data.timelines.achievements}
        dateField="unlocked_at"
      />
...
export function Row12Analytics({ data }: { data: Data }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <Card title="Achievement Analytics" icon={<Activity className="h-4 w-4 text-trophy" />}>
        <MiniSpark
          count={data.totals.achievementsEarned}
          max={Math.max(data.totals.achievementsCatalog, 1)}
          label="unlock rate"
        />
      </Card>
      <Card title="XP Analytics" icon={<Zap className="h-4 w-4 text-xp" />}>
        <MiniSpark count={data.xpRecent.length} max={30} label="recent xp events" />
      </Card>
      <Card title="Rank Analytics" icon={<Crown className="h-4 w-4 text-legendary" />}>
        <MiniSpark
          count={data.progression.currentRank?.rank_number ?? 0}
          max={Math.max(data.leaderboards.length || 1, 10)}
          label="rank standing"
        />
      </Card>
      <Card title="Reward Analytics" icon={<Gift className="h-4 w-4 text-primary" />}>
        <MiniSpark count={data.economy.claims.length} max={20} label="claim activity" />
      </Card>
    </div>
  );
}
...
export function Row13Halls({ data }: { data: Data }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
      <HallCard
        title="Hall of Fame"
        icon={<Star className="h-4 w-4 text-trophy" />}
        entries={h.fame}
        emptyHint="Hall of Fame populates from leaderboard champions."
      />
      <HallCard
        title="Champions Wall"
        icon={<Crown className="h-4 w-4 text-legendary" />}
        entries={h.champions}
        emptyHint="Configure champion rules in the Leaderboards page."
      />
      <HallCard
        title="Top 100"
        icon={<BarChart3 className="h-4 w-4 text-primary" />}
        entries={h.top100.slice(0, 8)}
        subtitle={h.top100.length ? `${h.top100.length} ranked` : undefined}
        emptyHint="No entries yet."
      />
      <HallCard
        title="Legends Board"
        icon={<Award className="h-4 w-4 text-mythic" />}
        entries={h.legends}
        emptyHint="Awaiting first legendary milestone."
      />
    </div>
  );
}
          ...
          {entries.map((e: any) => (
            <li key={e.user_id} className="flex items-center justify-between text-sm">
              ...
              <span className="flex-1 truncate ml-2">
                {e.profiles?.display_name ?? "Anonymous"}
              </span>
...
function RarityChip({ name, rarity }: { name?: string; rarity?: string }) {
  ...
  return (
    <div
      title={name}
      className={cn(
        "aspect-square rounded-lg bg-card flex items-center justify-center border",
        ring[rarity ?? "common"] ?? "border-border",
      )}
    >
...
function MissionsCard({
  ...
  return (
    <Card title={title} icon={icon}>
      ...
      {items.length === 0 ? (
        <EmptyHint>No active missions. Admins can add some in the Missions page.</EmptyHint>
      ) : (
...
function ListCard({ title, icon, items }: { title: string; icon: React.ReactNode; items: any[] }) {
  return (
    <Card title={title} icon={icon}>
      {items.length === 0 ? (
        <EmptyHint>Nothing active.</EmptyHint>
      ) : (
        <ul className="space-y-1.5">
          {items.slice(0, 5).map((i: any) => (
            <li key={i.id} className="flex items-center justify-between text-sm">
              ...
              {"xp_reward" in i && (
                <span className="text-xs text-xp tabular-nums">+{i.xp_reward}</span>
              )}
            </li>
          ))}
        </ul>
...
function TimelineCard({
  ...
  return (
    <Card title={title} icon={icon}>
      {items.length === 0 ? (
        <EmptyHint>Nothing recent.</EmptyHint>
      ) : (
        <ul className="space-y-2">
