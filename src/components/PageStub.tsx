          ...
          {sections.map((s) => (
            <div key={s} className="surface-card p-5">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Admin-managed module — full CRUD lands in the next milestone.
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="surface-card p-6 flex items-center gap-3">
        ...
        <div className="text-sm">
          ...
          <div className="text-xs text-muted-foreground">
            Schema and access control for this module are live in Lovable Cloud. Full admin CRUD UI
            (Add / Edit / Delete / Duplicate / Archive / Restore / Import / Export / Bulk) ships in
            Milestone 2.
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-wrap gap-2">
      {[
        ...
        <button
          key={a}
          disabled
          className="text-xs px-3 py-1.5 rounded-md border border-border bg-card/40 text-muted-foreground opacity-60 cursor-not-allowed"
        >
          {a}
        </button>
      ))}
