...
export const Route = createFileRoute("/ams/tickets/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Create, assign, work and close enterprise support tickets across every product.",
      },
...
function AmsListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      listTickets({ data: { status, priority: priority === "all" ? undefined : priority, q } }),
  });
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-2">
        {(
          ...
          >
            ...
            <div className="text-2xl font-display font-semibold text-gradient-trophy">
              {stats[k] ?? 0}
            </div>
          </button>
        ))}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <Input
            ...
            placeholder="Search by subject or AMS-…"
            className="pl-9 h-9"
          />
        </div>
                ...
                <tr key={t.id} className="hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs">
                    <Link
                      to="/ams/tickets/$id"
                      params={{ id: t.id }}
                      className="text-trophy hover:underline"
                    >
                      {t.ticket_no}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${PRIORITY_META[t.priority].tone}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${STATUS_META[t.status].tone}`}
                    >
