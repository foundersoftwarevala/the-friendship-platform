  ...
  head: () => ({
    meta: [
      { title: "Award Management — AMS" },
      {
        name: "description",
        content:
          "Create, edit, approve, publish and archive every award, trophy, badge, achievement and rank used across the Software Vala ecosystem.",
      },
            ...
            <TableBody>
              {rows.map((a) => (
                <TableRow key={a.id}>
                  ...
                  <TableCell className="text-right font-mono text-trophy">
                    +{a.rewards.xp.toLocaleString()}
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                          <Link to="/ams/awards/$id" params={{ id: a.id }}>
                            <Eye className="h-4 w-4 mr-2" /> View
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                          <Link to="/ams/awards/$id/edit" params={{ id: a.id }}>
                            <Pencil className="h-4 w-4 mr-2" /> Edit
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => run(() => cloneAward(a.id), "Cloned")}>
                          <Copy className="h-4 w-4 mr-2" /> Clone
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => run(() => approveAward(a.id), "Approved")}>
                          <Check className="h-4 w-4 mr-2" /> Approve
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => run(() => rejectAward(a.id), "Rejected")}>
                          <X className="h-4 w-4 mr-2" /> Reject
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => run(() => publishAward(a.id), "Published")}
                        >
                          <Send className="h-4 w-4 mr-2" /> Publish
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => run(() => unpublishAward(a.id), "Unpublished")}
                        >
                          <Ban className="h-4 w-4 mr-2" /> Unpublish
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => run(() => enableAward(a.id), "Enabled")}>
                          <Power className="h-4 w-4 mr-2" /> Enable
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => run(() => disableAward(a.id), "Disabled")}>
                          <Power className="h-4 w-4 mr-2" /> Disable
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => run(() => archiveAward(a.id), "Archived")}>
                          <Archive className="h-4 w-4 mr-2" /> Archive
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => run(() => restoreAward(a.id), "Restored")}>
                          <RotateCcw className="h-4 w-4 mr-2" /> Restore
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={() => run(() => deleteAward(a.id), "Deleted")}
