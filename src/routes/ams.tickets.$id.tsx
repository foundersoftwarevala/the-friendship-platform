  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center gap-2">
            <Select
              ...
              onValueChange={async (v) => {
                await changeStatus({ data: { id, to: v as AmsStatus } });
                toast.success("Status updated");
                invalidate();
              }}
            >
              <SelectTrigger className="h-9 w-44" aria-label="Ticket status">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {AMS_STATUSES.map((s) => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
        <Meta label="Status">
          <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_META[t.status].tone}`}
          >
        ...
        <Meta label="Priority">
          <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${PRIORITY_META[t.priority].tone}`}
          >
        ...
        <Meta label="Expected">
          {t.expected_resolution_at ? new Date(t.expected_resolution_at).toLocaleString() : "—"}
        </Meta>
              ...
              <ul className="divide-y divide-border">
                {data.events.map((e) => (
                  <li key={e.id} className="px-5 py-3 flex items-center gap-4 text-sm">
                    <div className="w-32 capitalize text-xs text-muted-foreground">
                      {e.kind.replace(/_/g, " ")}
                    </div>
          ...
          <div className="space-y-2">
            {data.comments.length === 0 ? (
              ...
              data.comments.map((c) => (
                <div
                  key={c.id}
                  className={`surface-card p-4 ${c.is_internal ? "ring-1 ring-amber-500/40" : ""}`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="text-xs text-muted-foreground">
                      {c.is_internal && (
                        <span className="inline-flex items-center gap-1 mr-2 text-amber-400">
              ...
              {data.attachments.map((a) => (
                <li key={a.id} className="px-4 py-3 flex items-center gap-3 text-sm">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="font-medium hover:text-trophy"
                  >
                    {a.file_name}
                  </a>
...
function CommentComposer({ id, onPosted }: { id: string; onPosted: () => void }) {
  ...
  return (
    <div className="surface-card p-4 space-y-3">
      ...
      />
      <p id={`comment-${id}-hint`} className="sr-only">
        Toggle internal note to keep this comment hidden from the customer.
      </p>
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={async () => {
            setBusy(true);
            try {
              await addComment({ data: { ticket_id: id, body, is_internal: internal } });
              setBody("");
            ...
            <div key={m.id} className="flex items-start gap-3">
              <div className="flex-1 rounded-lg bg-muted/30 p-3 text-sm">
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground mb-1">
                  ...
                  {m.pinned && (
                    <span className="text-trophy inline-flex items-center gap-1">
              ...
              <Button
                ...
                onClick={async () => {
                  await toggleChatPin({ data: { id: m.id, pinned: !m.pinned } });
                  onChanged();
                }}
              >
                <Pin
                  className={`h-3.5 w-3.5 ${m.pinned ? "text-trophy" : "text-muted-foreground"}`}
                />
              </Button>
            </div>
          ))
