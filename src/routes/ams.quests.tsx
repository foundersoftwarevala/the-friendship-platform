  ...
  head: () => ({
    meta: [
      { title: "Quests — AMS" },
      {
        name: "description",
        content:
          "Multi-stage quest chains — story mode, boss missions, elite challenges, season passes and XP journeys — with ordering and dependencies.",
      },
      ...
      <div className="surface-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            {quest.description && (
              <p className="text-sm text-muted-foreground mt-1 max-w-2xl">{quest.description}</p>
            )}
            {quest.season && (
              <Badge
                variant="outline"
                className="mt-2 bg-amber-500/10 text-primary border-amber-500/30"
              >
                {quest.season}
              </Badge>
            )}
...
function NewQuestDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <DialogContent className="max-w-2xl">
      <DialogHeader>
        <DialogTitle>New quest chain</DialogTitle>
      </DialogHeader>
      <div className="grid gap-4 py-2">
        <Field label="Name">
          <Input
            ...
            placeholder="The Founder's Ascent"
          />
        </Field>
  ...
  return (
    <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
      ...
      <div className="grid gap-4 py-2">
        ...
        <Field label="Order">
          <Input type="number" value={order} onChange={(e) => setOrder(+e.target.value || 1)} />
        </Field>
