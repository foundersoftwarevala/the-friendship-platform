  ...
  head: () => ({
    meta: [
      { title: "Missions — AMS" },
      {
        name: "description",
        content:
          "Daily, weekly, monthly, yearly, department, hidden and community missions — each with rules, activation windows and reward grants.",
      },
...
const STATUS_META: Record<MissionStatus, { label: string; className: string }> = {
  ...
  scheduled: {
    label: "Scheduled",
    className: "bg-blue-500/15 text-primary-glow border-blue-500/30",
  },
  active: {
    label: "Active",
    className: "bg-emerald-500/15 text-accent-emerald border-emerald-500/30",
  },
  paused: { label: "Paused", className: "bg-amber-500/15 text-primary border-amber-500/30" },
  completed: {
    label: "Completed",
    className: "bg-legendary/20 text-legendary border-legendary/40",
  },
  ...
  return (
    <div className="surface-card p-4 flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-1.5 text-[11px]">
        ...
        {m.rewards.coins > 0 && (
          <span className="px-2 py-0.5 rounded bg-yellow-500/10 text-accent-amber border border-yellow-500/30">
            +{m.rewards.coins} coins
          </span>
        )}
        {m.rewards.tokens > 0 && (
          <span className="px-2 py-0.5 rounded bg-primary/12 text-primary-glow border border-primary/30">
            +{m.rewards.tokens} tokens
          </span>
        )}
        {m.rewards.awardIds.length > 0 && (
          <span className="px-2 py-0.5 rounded bg-legendary/10 text-legendary border border-legendary/30">
            {m.rewards.awardIds.length} award{m.rewards.awardIds.length > 1 ? "s" : ""}
          </span>
        )}
      </div>
  ...
  return (
    <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
      ...
      <div className="grid gap-4 py-2">
        <Field label="Name">
          <Input
            ...
            placeholder="Daily login streak"
          />
        </Field>
        <Field label="Description">
          <Textarea
            ...
            placeholder="What the player must accomplish…"
          />
        </Field>
        ...
        <div className="rounded-md border border-border p-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Field label="XP">
              <Input type="number" value={xp} onChange={(e) => setXp(+e.target.value || 0)} />
            </Field>
            <Field label="Coins">
            ...
            <Field label="Tokens">
              <Input
                type="number"
                value={tokens}
                onChange={(e) => setTokens(+e.target.value || 0)}
              />
            </Field>
          </div>
        </div>
      ...
      <DialogFooter>
        ...
        <Button
          onClick={submit}
          className="bg-gradient-to-r from-trophy to-legendary text-background"
        >
          Create mission
        </Button>
      </DialogFooter>
