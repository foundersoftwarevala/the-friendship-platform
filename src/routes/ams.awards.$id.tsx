...
export const Route = createFileRoute("/ams/awards/$id")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "It may have been deleted or you don't have permission to view it.",
      },
...
function AwardDetailPage() {
  ...
  const action = useMutation({
    mutationFn: async (
      ...
      switch (op) {
        case "publish":
          return publishAward(id);
        case "unpublish":
          return unpublishAward(id);
        case "approve":
          return approveAward(id);
        case "reject":
          return rejectAward(id);
        case "enable":
          return enableAward(id);
        case "disable":
          return disableAward(id);
        case "archive":
          return archiveAward(id);
        case "restore":
          return restoreAward(id);
        case "clone":
          return cloneAward(id);
        case "delete":
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      <Button asChild variant="ghost" size="sm" className="gap-1.5 -ml-2">
        <Link to="/ams/awards">
          <ChevronLeft className="h-4 w-4" /> Back to awards
        </Link>
      </Button>
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button asChild variant="outline" className="gap-1.5">
              <Link to="/ams/awards/$id/edit" params={{ id: award.id }}>
                <Pencil className="h-4 w-4" /> Edit
              </Link>
            </Button>
            <Button variant="outline" className="gap-1.5" onClick={() => action.mutate("clone")}>
            ...
            {award.status === "published" ? (
              ...
              >
                <Ban className="h-4 w-4" /> Unpublish
              </Button>
            ) : (
              <Button
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent
          value="overview"
          className="mt-4 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6"
        >
        ...
        <TabsContent value="rules" className="mt-4 surface-card p-6 space-y-3">
          ...
          {award.unlockConditions.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Manual grant only — no automatic unlock conditions are defined.
            </p>
          ) : (
        ...
        <TabsContent
          value="rewards"
          className="mt-4 surface-card p-6 grid grid-cols-2 md:grid-cols-3 gap-4"
        >
          <Stat label="XP" value={`+${award.rewards.xp.toLocaleString()}`} />
          <Stat label="Coins" value={award.rewards.coins.toLocaleString()} />
          ...
          <Stat
            label="Monetary"
            value={`${award.rewards.monetaryValue.toLocaleString()} ${award.rewards.currency ?? ""}`}
          />
        </TabsContent>
        ...
        <TabsContent value="usage" className="mt-4">
          {award.usage.earnedCount === 0 ? (
            <EmptyState
              ...
              description="Once users start unlocking this award, usage stats appear here."
            />
          ) : (
