    ...
    return {
      meta: [
        { title: role ? `${role.name} — Sovereign Room` : "Role Showcase" },
        {
          name: "description",
          content: role
            ? `Immersive showcase for the ${role.name} identity: cover, avatar, passport, trophy, medal, badge, certificate.`
            : "Role showcase room.",
        },
  ...
  return (
    <div style={{ background: t.environment, fontFamily: t.bodyFont }} className="min-h-full">
      ...
      <section className="relative overflow-hidden">
        ...
        {/* volumetric light */}
        <div
          className="absolute -top-24 left-1/3 h-96 w-96 rounded-full"
          style={{
            background: `radial-gradient(closest-side, ${t.primary}55, transparent)`,
            filter: "blur(20px)",
          }}
        />
        <div
          className="absolute -bottom-24 right-1/4 h-80 w-80 rounded-full"
          style={{
            background: `radial-gradient(closest-side, ${t.accent}55, transparent)`,
            filter: "blur(24px)",
          }}
        />
        ...
        <div className="relative px-6 lg:px-10 pt-6 pb-16">
          <Link
            to="/ams/role-showcase"
            className="inline-flex items-center gap-1.5 text-xs text-foreground/80 hover:text-foreground transition"
          >
          ...
          <div className="mt-8 grid gap-8 md:grid-cols-[auto_1fr] items-end">
            ...
            <div>
              ...
              <h1
                className="mt-2 text-4xl lg:text-5xl text-foreground leading-none"
                style={{ fontFamily: t.displayFont }}
              >
                {role.name}
              </h1>
              <p className="mt-3 text-sm text-foreground/70 max-w-2xl italic">
                "{role.motto}" — {role.vision}
              </p>
              {/* Rank ribbon */}
              <div
                className="mt-4 inline-flex items-center gap-3 rounded-full pl-1 pr-4 py-1 border"
                style={{ borderColor: `${t.primary}66`, background: `${t.primary}12` }}
              >
                <span
                  className="rounded-full px-2.5 py-1 text-[10px] font-mono uppercase tracking-widest"
                  style={{ background: t.metallic, color: t.ink }}
                >
                  {tier.key}
                </span>
                <span
                ...
                <span className="text-[10px] font-mono" style={{ color: `${t.primary}bb` }}>
                  {holderId}
                </span>
              </div>
            </div>
          ...
          <div className="mt-6 flex flex-wrap gap-1.5">
            ...
            <button
              ...
              style={{
                background: `linear-gradient(135deg, ${t.primary}, ${tier.hue})`,
                color: t.ink,
      ...
      <div className="px-6 lg:px-10 py-10 space-y-12">
        ...
        <Row
          theme={t}
          label="Identity"
          caption="Membership card · Digital passport · Verification seal"
        >
        ...
        <Row theme={t} label="Trophy" caption={`${role.trophyStyle} · ${tier.label}`}>
          <div className="grid gap-6 lg:grid-cols-3">
            <div
              className="lg:col-span-1 relative rounded-2xl border overflow-hidden aspect-square flex items-center justify-center"
              style={{
            ...
            >
              ...
              {/* Spotlight */}
              <div
                className="absolute top-0 left-1/2 -translate-x-1/2 h-40 w-40 rounded-full"
                style={{
                  background: `radial-gradient(closest-side, ${tier.hue}77, transparent)`,
                  filter: "blur(10px)",
                }}
              />
              <div
                className="relative trophy-float"
                style={{ filter: `drop-shadow(0 20px 30px ${t.glow})` }}
              >
                <StageTrophy
                  shape={t.trophyShape}
                  accent={tier.hue}
                  id={`sr-${t.slug}-${tierIdx}`}
                  className="h-64 w-64"
                />
              </div>
              {/* Display base label */}
              <div
                className="absolute bottom-6 left-0 right-0 text-center text-[10px] tracking-[0.3em] uppercase"
                style={{ color: `${tier.hue}dd`, fontFamily: t.displayFont }}
              >
                {tier.label}
              </div>
            </div>
            <div className="lg:col-span-2 space-y-4">
              <AwardPlate
                theme={t}
                title={role.awardExamples[Math.min(tierIdx, role.awardExamples.length - 1)]}
                subtitle={`Presented at the ${t.ceremony}`}
              />
                ...
                {role.awardExamples.slice(0, 4).map((a) => (
                  ...
                  >
                    <div
                      ...
                      style={{ background: t.metallic, color: t.ink, fontFamily: t.displayFont }}
                    >
                      {t.glyphIcon}
                    </div>
                    <div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-6 justify-center">
                {role.trophies.slice(0, 5).map((tt) => (
                  <Medal
                    key={tt.key}
                    theme={{ ...t, ribbon: [tt.hue, t.ink] as [string, string] }}
                    label={tt.key}
                  />
                ))}
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 justify-items-center">
                {role.badges.slice(0, 6).map((b) => (
                  <div key={b.key} className="flex flex-col items-center gap-1.5">
                    ...
                    <div className="text-[10px] text-foreground/70 text-center max-w-[8rem] leading-tight">
                      {b.label}
                    </div>
                  </div>
                ))}
        ...
        <Row
          ...
          caption="Glass shelves · Ambient lighting · Slow rotation"
        >
        ...
        <Row theme={t} label="Hall of Fame" caption="Legacy museum wall">
          <div
            ...
            style={{
              borderColor: `${t.primary}44`,
              background: `linear-gradient(180deg, ${t.paper}, ${t.ink})`,
            }}
          >
            <RolePattern
              kind={t.pattern}
            ...
            {role.reputationPillars.map((p) => (
              ...
              >
                <span
                  ...
                  style={{ background: t.primary, boxShadow: `0 0 10px ${t.glow}` }}
                />
...
function Row({
  ...
  theme: (typeof ROLE_THEMES)[RoleSlug];
  label: string;
  caption?: string;
    ...
    <section>
      <div className="mb-4 flex items-baseline gap-3">
        <div
          ...
          style={{ background: `linear-gradient(90deg, ${theme.primary}55, transparent)` }}
        />
        <div
          className="text-[10px] tracking-[0.4em] uppercase"
          style={{ color: `${theme.primary}dd`, fontFamily: theme.displayFont }}
        >
          {label}
        </div>
        <div
          className="h-px flex-[3]"
          style={{ background: `linear-gradient(90deg, transparent, ${theme.primary}22)` }}
        />
      </div>
