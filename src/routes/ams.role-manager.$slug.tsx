  ...
  return (
    <div className="p-12 text-center">
      ...
      <Button asChild variant="outline" className="mt-4">
        <Link to="/ams/role-manager">Back to Role Manager</Link>
      </Button>
    </div>
  );
}
...
function Chip({ children, hue }: { children: React.ReactNode; hue?: string }) {
  return (
    <span
      ...
      style={{
        borderColor: hue ? `${hue}55` : undefined,
        color: hue,
        background: hue ? `${hue}12` : undefined,
      }}
    >
      {children}
    </span>
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      <Button asChild variant="ghost" size="sm" className="gap-1.5 -ml-2">
        <Link to="/ams/role-manager">
          <ChevronLeft className="h-4 w-4" /> Back to Role Manager
        </Link>
      </Button>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="text-right space-y-1">
            ...
            <div className="font-mono text-lg" style={{ color: role.accent }}>
              {role.passportPrefix}-00001
            </div>
            <Badge variant="outline" className="mt-1">
              {role.passport.verification}
            </Badge>
          </div>
        </div>
      </header>
      ...
      <Section icon={Quote} kicker="Personality" title="Voice, tone and philosophy">
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          {[
            ...
            ["Congratulations", role.congratulations],
            ["Celebration", role.celebration],
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1 text-xs">
            {role.careerPath.map((c, i) => (
              <span key={c} className="inline-flex items-center gap-1">
                <span className="rounded border border-border/60 bg-muted/20 px-1.5 py-0.5">
                  {c}
                </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
          {role.trophies.map((t) => (
            <div
              ...
              style={{
                borderColor: `${t.hue}44`,
                background: `linear-gradient(180deg, ${t.hue}18, transparent)`,
              }}
            >
              ...
              <div className="text-xs font-semibold" style={{ color: t.hue }}>
                {t.label}
              </div>
            </div>
          ))}
          ...
          <ul className="space-y-2 text-sm">
            {role.awardExamples.map((a) => (
              <li key={a} className="flex items-center gap-2">
                <Star className="h-3.5 w-3.5" style={{ color: role.accent }} />
                {a}
              </li>
            ))}
          </ul>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {role.badges.map((b) => (
              <div key={b.key} className="rounded-lg border border-border/60 bg-muted/10 p-2.5">
                ...
                <div className="mt-0.5 font-medium" style={{ color: role.accent }}>
                  {b.label}
                </div>
              </div>
            ))}
      ...
      <Section icon={BookOpen} kicker="Certificates" title="Recognition Levels">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-2">
          {role.certificates.map((c) => (
            <div
              key={c.key}
              className="rounded-lg border border-border/60 bg-muted/10 p-3 text-center"
            >
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                {c.key}
              </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div
            ...
            style={{
              ...
              background: `linear-gradient(135deg, ${role.accent}22, transparent 70%), #0b0d12`,
            }}
          >
            <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">
              Software Vala
            </div>
            <div className="text-2xl font-bold mt-1" style={{ color: role.accent }}>
              {role.name} Passport
            </div>
            ...
            <div
              className="absolute right-4 bottom-4 text-4xl opacity-60"
          ...
          {role.journey.map((s, i) => (
            <li key={s.key} className="pl-4 relative">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="font-semibold text-sm" style={{ color: role.accent }}>
                  {s.label}
                </span>
              </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <ul className="space-y-1.5 text-sm">
            {role.reputationPillars.map((p) => (
              <li key={p} className="flex items-center gap-2">
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: role.accent }} />
                {p}
              </li>
            ))}
          </ul>
        ...
        <Section icon={Trophy} kicker="Legacy" title="The Line You Leave">
          <p className="text-sm italic" style={{ color: role.accent }}>
            {role.legacyLine}
          </p>
          <div className="mt-3 text-[10px] uppercase tracking-wider text-muted-foreground">
            Digital Museum · Hall of Fame · Lifetime Journey
          </div>
        </Section>
      </div>
    </div>
